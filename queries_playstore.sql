...
SELECT app_name FROM analytics WHERE min_installs <= 50 AND rating <> null ORDER By rating DESC;

-- 9. Find the names of all apps that are rated less than 3 with at least 10000 reviews.
SELECT app_name FROM analytics WHERE rating < 3 AND reviews > 10000;

-- 10. Find the top 10 most-reviewed apps that cost between 10 cents and a dollar.
SELECT app_name, price FROM analytics WHERE price > .10 AND price < 1 ORDER BY reviews DESC;
...
SELECT app_name, reviews, min_installs AS installs, min_installs/reviews AS proportion FROM analytics WHERE min_installs > 100000 ORDER BY proportion DESC LIMIT 1;